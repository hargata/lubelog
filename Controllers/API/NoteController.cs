    ...
    public partial class APIController
    {
        ...
        public IActionResult AddNote(int vehicleId, NoteRecordExportModel input)
        {
            ...
            try
            {
                ...
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(),WebHookPayload.FromNoteRecord(note, "note.add.api", User.Identity?.Name ?? string.Empty));
        ...
        public IActionResult DeleteNote(int id)
        {
            ...
            if (result)
            {
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromNoteRecord(existingRecord, "note.delete.api", User.Identity?.Name ?? string.Empty));
            }
        ...
        public IActionResult UpdateNote(NoteRecordExportModel input)
        {
            ...
            try
            {
                ...
                if (existingRecord != null && existingRecord.Id == int.Parse(input.Id))
                {
                    ...
                    StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromNoteRecord(existingRecord, "note.update.api", User.Identity?.Name ?? string.Empty));
                }
                else
                {
                    Response.StatusCode = 400;
