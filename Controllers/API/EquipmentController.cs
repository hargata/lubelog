    ...
    public partial class APIController
    {
        ...
        public IActionResult AddEquipmentRecord(int vehicleId, EquipmentRecordExportModel input)
        {
            ...
            try
            {
                ...
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromEquipmentRecord(equipmentRecord, "equipmentrecord.add.api", User.Identity?.Name ?? string.Empty));
        ...
        public IActionResult DeleteEquipmentRecord(int id)
        {
            ...
            if (result)
            {
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromEquipmentRecord(existingRecord, "equipmentrecord.delete.api", User.Identity?.Name ?? string.Empty));
            }
        ...
        public IActionResult UpdateEquipmentRecord(EquipmentRecordExportModel input)
        {
            ...
            try
            {
                ...
                if (existingRecord != null && existingRecord.Id == int.Parse(input.Id))
                {
                    ...
                    StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromEquipmentRecord(existingRecord, "equipmentrecord.update.api", User.Identity?.Name ?? string.Empty));
                }
                else
                {
                    Response.StatusCode = 400;
