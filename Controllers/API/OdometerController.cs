    ...
    public partial class APIController
    {
        ...
        public IActionResult AddOdometerRecord(int vehicleId, OdometerRecordExportModel input, bool autoIncludeEquipment = false)
        {
            ...
            try
            {
                ...
                _eventLogic.PublishEvent(WebHookPayload.FromOdometerRecord(odometerRecord, "odometerrecord.add.api", User.Identity?.Name ?? string.Empty));
        ...
        public IActionResult DeleteOdometerRecord(int id)
        {
            ...
            if (result)
            {
                _eventLogic.PublishEvent(WebHookPayload.FromOdometerRecord(existingRecord, "odometerrecord.delete.api", User.Identity?.Name ?? string.Empty));
            }
        ...
        public IActionResult UpdateOdometerRecord(OdometerRecordExportModel input)
        {
            ...
            try
            {
                ...
                if (existingRecord != null && existingRecord.Id == int.Parse(input.Id))
                {
                    ...
                    _eventLogic.PublishEvent(WebHookPayload.FromOdometerRecord(existingRecord, "odometerrecord.update.api", User.Identity?.Name ?? string.Empty));
                }
                else
                {
                    Response.StatusCode = 400;
