    ...
    public partial class APIController
    {
        ...
        public IActionResult AddUpgradeRecord(int vehicleId, GenericRecordExportModel input)
        {
            ...
            try
            {
                ...
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromGenericRecord(upgradeRecord, "upgraderecord.add.api", User.Identity?.Name ?? string.Empty));
        ...
        public IActionResult DeleteUpgradeRecord(int id)
        {
            ...
            if (result)
            {
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromGenericRecord(existingRecord, "upgraderecord.delete.api", User.Identity?.Name ?? string.Empty));
            }
        ...
        public IActionResult UpdateUpgradeRecord(GenericRecordExportModel input)
        {
            ...
            try
            {
                ...
                if (existingRecord != null && existingRecord.Id == int.Parse(input.Id))
                {
                    ...
                    StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromGenericRecord(existingRecord, "upgraderecord.update.api", User.Identity?.Name ?? string.Empty));
                }
                else
                {
                    Response.StatusCode = 400;
