    ...
    public partial class APIController
    {
        ...
        public IActionResult AddTaxRecord(int vehicleId, TaxRecordExportModel input)
        {
            ...
            try
            {
                ...
                _eventLogic.PublishEvent(WebHookPayload.FromTaxRecord(taxRecord, "taxrecord.add.api", User.Identity?.Name ?? string.Empty));
        ...
        public IActionResult DeleteTaxRecord(int id)
        {
            ...
            if (result)
            {
                _eventLogic.PublishEvent(WebHookPayload.FromTaxRecord(existingRecord, "taxrecord.delete.api", User.Identity?.Name ?? string.Empty));
            }
        ...
        public IActionResult UpdateTaxRecord(TaxRecordExportModel input)
        {
            ...
            try
            {
                ...
                if (existingRecord != null && existingRecord.Id == int.Parse(input.Id))
                {
                    ...
                    _eventLogic.PublishEvent(WebHookPayload.FromTaxRecord(existingRecord, "taxrecord.update.api", User.Identity?.Name ?? string.Empty));
                }
                else
                {
                    Response.StatusCode = 400;
