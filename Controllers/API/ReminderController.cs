    ...
    public partial class APIController
    {
        ...
        public IActionResult AddReminderRecord(int vehicleId, ReminderExportModel input)
        {
            ...
            try
            {
                ...
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromReminderRecord(reminderRecord, "reminderrecord.add.api", User.Identity?.Name ?? string.Empty));
        ...
        public IActionResult UpdateReminderRecord(ReminderExportModel input)
        {
            ...
            try
            {
                ...
                if (existingRecord != null && existingRecord.Id == int.Parse(input.Id))
                {
                    ...
                    StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromReminderRecord(existingRecord, "reminderrecord.update.api", User.Identity?.Name ?? string.Empty));
                }
                else
                {
                    Response.StatusCode = 400;
        ...
        public IActionResult DeleteReminderRecord(int id)
        {
            ...
            if (result)
            {
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromReminderRecord(existingRecord, "reminderrecord.delete.api", User.Identity?.Name ?? string.Empty));
            }
