...
namespace CarCareTracker.Controllers
{
    public partial class VehicleController
    {
        ...
        private OperationResponse DeleteInspectionRecordWithChecks(int inspectionRecordId)
        {
            ...
            if (result)
            {
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromInspectionRecord(existingRecord, "inspectionrecord.delete", User.Identity?.Name ?? string.Empty));
            }
            return OperationResponse.Conditional(result, string.Empty, StaticHelper.GenericErrorMessage);
        }
        [HttpPost]
        ...
        public IActionResult SaveInspectionRecordToVehicleId(InspectionRecordInput inspectionRecord)
        {
            ...
            if (result)
            {
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromInspectionRecord(convertedRecord, "inspectionrecord.add", User.Identity?.Name ?? string.Empty));
            }
        ...
        public IActionResult UpdateInspectionRecord(InspectionRecordInput inspectionRecord)
        {
            ...
            if (result)
            {
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromInspectionRecord(existingRecord, "inspectionrecord.update", User.Identity?.Name ?? string.Empty));
            }
            return Json(OperationResponse.Conditional(result, string.Empty, StaticHelper.GenericErrorMessage));
        }
    }
