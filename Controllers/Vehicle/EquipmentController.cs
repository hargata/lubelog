...
namespace CarCareTracker.Controllers
{
    public partial class VehicleController
    {
        ...
        public IActionResult SaveEquipmentRecordToVehicleId(EquipmentRecordInput equipmentRecord)
        {
            ...
            if (result)
            {
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromEquipmentRecord(convertedRecord, equipmentRecord.Id == default ? "equipmentrecord.add" : "equipmentrecord.update", User.Identity?.Name ?? string.Empty));
            }
        ...
        private OperationResponse DeleteEquipmentRecordWithChecks(int equipmentRecordId)
        {
            ...
            if (result)
            {
                StaticHelper.NotifyAsync(_config.GetWebHookUrl(), WebHookPayload.FromEquipmentRecord(existingRecord, "equipmentrecord.delete", User.Identity?.Name ?? string.Empty));
            }
